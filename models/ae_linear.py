...
class Encoder(nn.Module):
    def __init__(self, n_channels, w_shape, h_shape, out_dim):
        ...
        self.fc1   = nn.Linear(n_channels*w_shape*h_shape, 1024)
...
class Decoder(nn.Module):
    def __init__(self, in_dim, h_shape, out_dim):
        super(Decoder, self).__init__()
        self.h_shape, self.channels = h_shape, out_dim
#        self.h_shape, self.in_channels = 415, 1
        ...
        self.fc3   = nn.Linear(512, h_shape*out_dim)
...
class AELinear(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = Decoder(in_dim=hidden_dim, out_dim=config.n_channels, h_shape=config.window_len)
