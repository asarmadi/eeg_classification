...
class Conv1D(nn.Module):
    ...
    def __init__(self, config, kernel_size):
        ...
        self.conv_layer1 = nn.Sequential(nn.Conv1d(config.n_channels, 2*config.n_channels, kernel_size=kernel_size),nn.BatchNorm1d(2*config.n_channels),nn.ReLU(inplace=True))
        out_shape = (config.window_len-kernel_size+1)

        self.conv_layer2 = nn.Sequential(nn.Conv1d(2*config.n_channels, 4*config.n_channels, kernel_size=kernel_size),nn.BatchNorm1d(4*config.n_channels),nn.AvgPool2d(2),nn.ReLU(inplace=True))
